...
import { Champion, Player, useMatchHistory } from '../data/matchHistory';

export async function loader(data: { params: any }) {
    return data.params.matchId;
}

const MatchPlayerCard = React.memo(({ player }: { player: Player }) => {
    ...
    return (
        <Flex
            ...
            maxWidth={180}
        >
            <Button variant='ghost' onClick={championNav} flex={1} padding={1}>
                <Image src={player.champion.images.portrait} />
            </Button>
            <Button variant='ghost' onClick={playerNav} alignSelf={'stretch'}>
                <Text
...
const BannedChampion = React.memo(({ champion }: { champion: Champion }) => {
    const navigate = useNavigate();
    ...
    return (
        <Flex width={12} height={12} justify={'center'} marginRight={2}>
            <Button variant='ghost' onClick={championNav} padding={1}>
                <Image
...
export const MatchScreen = React.memo(function MatchHistory() {
    ...
    const matchHistory = useMatchHistory().map((match) => {
        return {
            ...match,
